...
class OrderEventProducerTest {
    ...
    private KafkaMessageListenerContainer<String, OrderEvent>
            container;
    private BlockingQueue<ConsumerRecord<String, OrderEvent>>
            records;
    ...
    void setUp() {
        ...
        ContainerProperties containerProperties =
                new ContainerProperties(orderTopic);
        ...
        container.setupMessageListener(
                (MessageListener<String, OrderEvent>)
                records::add);
    ...
    void shouldSendOrderEventSuccessfully()
        ...
        assertThat(received.value().getOrderId())
                .isEqualTo(orderId);
        assertThat(received.value().getCustomerId())
                .isEqualTo("CUSTOMER-001");
        assertThat(received.value().getQuantity()).isEqualTo(2);
        assertThat(received.value().getTotalAmount())
                .isEqualByComparingTo("59.98");
        assertThat(received.value().getStatus())
                .isEqualTo(OrderEvent.OrderStatus.CREATED);
    }

    @Test
    @DisplayName("Should send multiple order events successfully")
    void shouldSendMultipleOrderEvents()
            throws InterruptedException {

        String orderId1 = "ORDER-A-" + System.currentTimeMillis();
        String orderId2 = "ORDER-B-" + System.currentTimeMillis();

        OrderEvent event1 = OrderEvent.of(
                orderId1, "CUSTOMER-002",
                "PRODUCT-002", 1,
                new BigDecimal("29.99"));

        OrderEvent event2 = OrderEvent.of(
                orderId2, "CUSTOMER-003",
                "PRODUCT-003", 3,
        ...
        ConsumerRecord<String, OrderEvent> received1 =
                records.poll(10, TimeUnit.SECONDS);
        ConsumerRecord<String, OrderEvent> received2 =
                records.poll(10, TimeUnit.SECONDS);

        assertThat(received1).isNotNull();
        assertThat(received2).isNotNull();

        java.util.List<String> receivedIds = java.util.Arrays.asList(
                received1.value().getOrderId(),
    ...
    void shouldUseOrderIdAsMessageKey()
            throws InterruptedException {

        String orderId = "ORDER-KEY-" + System.currentTimeMillis();

        OrderEvent event = OrderEvent.of(
                orderId, "CUSTOMER-004",
                "PRODUCT-004", 1,
